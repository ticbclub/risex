...
def get_wallet(account):
    ...
    # Equity curve + smart-money flag + PnL por ventana corta
    equity_curve = None; max_dd_30d = None; smart = False
    pnl_1d = None; pnl_7d = None
    try:
        with _VOL_LOCK:
            v2 = _VOL["by_account"].get(account)
        if v2:
            equity_curve = v2.get("equity_curve") or None
            max_dd_30d = v2.get("max_dd_30d")
            pnl_1d = v2.get("pnl_1d"); pnl_7d = v2.get("pnl_7d")
            smart = _is_smart_money(v2)
    except Exception:
        pass
        ...
        for t in trades:
            ...
            if ts >= earliest_cutoff:
                ...
                if ts >= cutoffs["30d"]:
                    ...
                    out["fees_30d"] += fee
                    if rp > 0: out["wins_30d"] += 1
                    elif rp < 0: out["losses_30d"] += 1
                    if bool(t.get("is_liquidation")):
                        out["n_liquidations"] += 1
                    if rp != 0:
                        pnl_events.append((ts, rp))
                if ts >= cutoffs["7d"]:
                    out["pnl_7d"] += rp; out["fees_7d"] += fee; out["trades_7d"] += 1
                if ts >= cutoffs["1d"]:
                    out["pnl_1d"] += rp; out["fees_1d"] += fee; out["trades_1d"] += 1
...
def _share_page_html(kind, ident, og_path, title, desc, deeplink):
    ...
    return (
        ...
        f'<div style="text-align:center"><h1>Loading RISEx Stats…</h1>'
...
def get_pnl_ranking(period="30d", limit=100):
    """Ranking de top traders por PnL realizado en una ventana.
       period in {"1d", "7d", "30d", "all"}. "all" agrega realized 30d + unrealized actual.
       Devuelve también edge_bps = pnl/volume*10000 y flag smart_money.
    """
    if period not in ("1d", "7d", "30d", "all"):
        period = "30d"
    # unrealized PnL por cuenta (snapshot actual del indexer de posiciones)
    unreal = {}
    with _IDX_LOCK:
    ...
    with _VOL_LOCK:
        for a, v in _VOL["by_account"].items():
            if period == "1d":
                rp = v.get("pnl_1d", 0.0); vol = v.get("1d", 0.0); tr = v.get("trades_1d", 0)
                fee = v.get("fees_1d", 0.0); up = 0.0
            elif period == "7d":
                rp = v.get("pnl_7d", 0.0); vol = v.get("7d", 0.0); tr = v.get("trades_7d", 0)
                fee = v.get("fees_7d", 0.0); up = 0.0
            elif period == "30d":
                rp = v.get("realized_pnl_30d", 0.0); vol = v.get("30d", 0.0); tr = v.get("trades", 0)
                fee = v.get("fees_30d", 0.0); up = 0.0
            else:  # all
                rp = v.get("realized_pnl_30d", 0.0); vol = v.get("30d", 0.0); tr = v.get("trades", 0)
                fee = v.get("fees_30d", 0.0); up = unreal.get(a, 0.0)
            total = rp + up
            if rp == 0.0 and up == 0.0:
                continue
            edge_bps = (rp / vol * 10000) if vol > 0 else 0.0
            rows.append({"account": a, "realized": rp, "unrealized": up, "total": total,
                          "volume": vol, "fees": fee, "trades": tr,
                          "edge_bps": round(edge_bps, 1),
                          "smart": _is_smart_money(v),
                          "n_liquidations": v.get("n_liquidations", 0)})
    ...
    def do_GET(self):
        ...
        try:
            if path.path == "/api/data":
                ...
                if not mid.isdigit():
                    self._json({"ok": False, "error": "market_id required"}, 400); return
                self._json(get_candles(mid, interval=iv, limit=lim))
...
<p class="lead">RISEx Stats (<code>risexscan.io</code>) is a community-built real-time analytics dashboard for RISEx, the perpetual futures DEX on RISE chain. Built from public data only — no accounts, no tracking, no premium tiers.</p>
...
<p>If RISEx Stats has been useful for you, the best support is to use the <b>"Trade on RISEx" button</b> when you next open a position. It uses the creator's referral link and helps cover hosting + dev costs. Zero added cost for you, fully transparent.</p>
...
<div class="footer">RISEx Stats · risexscan.io · Built on public RISE chain data · Open source · No accounts, no tracking.</div>
</div></body></html>"""


METHODOLOGY_HTML = """<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Methodology · RISEx Stats</title>
<meta name="description" content="How RISEx Stats computes every metric — sources, formulas, edge cases.">
...
<html lang="en"><head><meta charset="utf-8"><title>404 · RISEx Stats</title>
...
<title>RISEx Stats · Live perp DEX analytics on RISE chain</title>
...
<meta name="keywords" content="RISEx, RISE chain, perp DEX, perpetual futures, crypto analytics, trading dashboard, smart money, liquidations, funding rates, on-chain analytics, DeFi, perpetuals">
<meta name="author" content="risexscan.io">
<meta name="theme-color" content="#00ffd4">
<link rel="canonical" href="https://risexscan.io/">
<!-- Open Graph / Social cards -->
<meta property="og:type" content="website">
<meta property="og:title" content="RISEx Stats · Live perp DEX analytics on RISE chain">
...
<meta property="og:site_name" content="RISEx Stats">
<meta property="og:image" content="https://risexscan.io/og/home.svg">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="RISEx Stats · Live perp analytics">
...
<script type="application/ld+json">
{
 ...
 "creator":{"@type":"Organization","name":"RISEx Stats","url":"https://risexscan.io"}
}
</script>
...
/* Price ticker band */
.priceticker{position:relative;display:flex;align-items:center;height:30px;background:linear-gradient(180deg,rgba(0,255,212,.04),transparent);border-bottom:1px solid var(--line2);overflow:hidden}
.priceticker .pt-track{display:flex;gap:32px;padding:0 20px;animation:pttick 60s linear infinite;white-space:nowrap;font-family:var(--mono);font-size:11.5px;font-weight:600}
.pt-asset{display:inline-flex;align-items:center;gap:8px;color:var(--muted)}
.pt-asset .pt-sym{color:var(--txt);font-weight:700;letter-spacing:.3px}
.pt-asset .pt-px{color:var(--accent)}
.pt-asset svg.pt-sk{width:42px;height:14px;display:inline-block;vertical-align:middle}
...
<div class="tour-back" id="tour_back">
 <div class="tour" id="tour_box">
  ...
  <h2 id="tour_title">Welcome to RISEx Stats</h2>
...
<div id="splash">
 <div class="splash-logo"><svg viewBox="0 0 252 303"><path d="M176.12 0.39H0.59V50.72H176.12C189.97 50.72 201.20 61.98 201.20 75.88V101.04H77.36C34.96 101.04 0.59 135.41 0.59 177.81V302.34H50.74V184.00L177.66 302.33H251.36L89.42 151.37H201.20V101.29H251.36V75.88C251.36 34.19 217.66 0.39 176.12 0.39Z" fill="#00ffd4"/></svg></div>
 <div class="splash-text">RISEX · STATS</div>
</div>
<div class="app">
<aside class="sidebar" id="sidebar">
 <div class="brand">
  <div class="logo"><svg viewBox="0 0 252 303" xmlns="http://www.w3.org/2000/svg"><path d="M176.12 0.39H0.59V50.72H176.12C189.97 50.72 201.20 61.98 201.20 75.88V101.04H77.36C34.96 101.04 0.59 135.41 0.59 177.81V302.34H50.74V184.00L177.66 302.33H251.36L89.42 151.37H201.20V101.29H251.36V75.88C251.36 34.19 217.66 0.39 176.12 0.39Z" fill="#fff"/></svg></div>
  <div><div class="name">RISEx Stats <span class="vbadge">v5</span></div><div class="sub">Live perp analytics</div></div>
 </div>
 <nav>
  <div class="navgroup">
   <div class="navtitle">Overview</div>
   <div class="navitem active" data-v="overview"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/><rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/></svg></span>Overview</div>
   <div class="navitem" data-v="markets"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 17 9 11 13 15 21 7"/><polyline points="14 7 21 7 21 14"/></svg></span>Markets</div>
  </div>
  <div class="navgroup">
   <div class="navtitle">Rankings</div>
   <div class="navitem" data-v="ranking"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="M5 7l7-4 7 4"/><circle cx="5" cy="7" r="1.5"/><circle cx="19" cy="7" r="1.5"/><path d="M3 11l2 4 2-4"/><path d="M17 11l2 4 2-4"/></svg></span>Positions</div>
   ...
   <div class="navitem" data-v="pnl"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M8 21h8"/><path d="M12 17v4"/><path d="M6 4h12v5a6 6 0 0 1-12 0V4z"/><path d="M18 6h2a2 2 0 0 1 0 4h-1"/><path d="M6 6H4a2 2 0 0 0 0 4h1"/></svg></span>Top PnL</div>
  ...
  <div class="navgroup">
   ...
   <div class="navitem" data-v="users"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span>Users</div>
  </div>
  <div class="navgroup">
   ...
   <div class="navitem" data-v="tools"><span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a3 3 0 0 0 4.2 4.2L21 12.4a8 8 0 0 1-11.3 9.3L3 15l3-3 6.7 6.7 1.7-1.7L8 10l4-4 6.7 6.7 1.7-1.7L13 4l1.7-1.7"/></svg></span>Tools</div>
...
<div class="view active" id="v_overview">
 <div id="wif_panel" class="wif-grid"></div>
 <div class="hero">
 ...
 <div class="panel" style="margin-top:14px"><h2>🐋 Whale Trades · last 24h</h2>
  <div class="row" style="gap:14px;align-items:center;margin-bottom:10px">
   <span class="seg" id="bt_min">
    <button data-m="50000">≥ $50k</button>
    <button data-m="100000" class="active">≥ $100k</button>
    <button data-m="250000">≥ $250k</button>
    <button data-m="1000000">≥ $1M</button>
   </span>
   <span class="seg" id="bt_sort">
    <button data-s="size" class="active">Biggest</button>
    <button data-s="time">Most recent</button>
   </span>
   <span class="note" id="bt_totals" style="margin-left:auto"></span>
  </div>
  <div style="max-height:420px;overflow:auto"><table><thead><tr>
   <th>Time</th><th>Trader</th><th>Market</th><th>Side</th><th>Size</th><th>Price</th><th>Notional</th><th>Role</th></tr></thead>
   <tbody id="bt_body"></tbody></table></div>
  <div class="note" style="margin-top:8px">Big trades captured during indexer passes. Liquidations excluded — see the Liquidations tab for those.</div>
 </div>
</div>
...
function renderTicker(){
 ...
 for(const sym of wanted){
  ...
  if(!m)continue;
  items.push({sym:sym,price:m.last_price,chg:m.change_24h,market_id:m.market_id});
 }
...
function renderCmdK(q){
 ...
 if(matches.length){
  ...
  for(const m of matches){
   ...
   html+=`<div class="cmdk-item" data-i="${i}"><span class="ic">📈</span><span class="lab">${m.name}</span><span class="sub">$${P(m.last_price)} · ${m.change_24h>=0?'+':''}${m.change_24h.toFixed(2)}%</span></div>`;
  }
 }
 // views
...
function renderWhatsInteresting(){
 ...
 if(ms.length){
  const mover=[...ms].sort((a,b)=>Math.abs(b.change_24h)-Math.abs(a.change_24h))[0];
  const cls=mover.change_24h>=0?'pos':'neg';
  cards.push({lbl:'Biggest mover',val:mover.name,sub:`<span class="${cls}">${mover.change_24h>=0?'+':''}${mover.change_24h.toFixed(2)}%</span>`,mid:mover.market_id});
 }
...
const TOUR=[
 {title:'Welcome to RISEx Stats',body:'The real-time analytics dashboard for RISE chain perps. Built on public data. No accounts, no tracking. Let me show you around.'},
...
function tourNext(){_tourStep++;if(_tourStep>=TOUR.length){closeTour();toast('Welcome aboard 🚀','ok');return;}renderTour();}
...
function renderMarkets(){
 ...
 for(const r of m){const cc=r.change_24h>=0?'pos':'neg',fd=r.funding_8h*100,fc=fd>=0?'pos':'neg',
  ...
  tr.innerHTML=`<td class="mkt"><span class="pinstar ${isPin?'pinned':''}" onclick="event.stopPropagation();togglePin('${r.market_id}')">★</span>${r.name} <span style="color:var(--muted);font-size:11px">↗</span></td>
   ...
   <td><span class="chgpill ${cc}">${r.change_24h>=0?'+':''}${r.change_24h.toFixed(2)}%</span></td>
...
async function loadWallet(addr){
 ...
 try{const [w,st]=await Promise.all([
   ...
   fetch('/api/wallet-stats?account='+encodeURIComponent(addr)).then(r=>r.json())
  ]);
  if(!w.ok){out.innerHTML='<div class="empty">'+(w.errors||['Error']).join(' ')+'</div>';return;}
  w.stats=st && st.ok ? st : null;
...
async function loadHistory(){
 try{const d=await (await fetch('/api/history')).json();const p=d.points||[];
  const lab=p.map(x=>new Date(x.t*1000).toLocaleString('en-US',{day:'2-digit',hour:'2-digit',minute:'2-digit'}));
  lineChart('hVol',lab,p.map(x=>x.vol),'rgb(51,214,166)');
  lineChart('hOi',lab,p.map(x=>x.oi),'rgb(80,221,194)');
...
function renderLiq(){
 ...
 // Banner with totals
 const banner=heroBanner({icon:'💀',label:'HALL OF PAIN · LAST 24H',value:'−'+U(d.total_liq_loss||0),
  meta:`${d.count||0} liquidations · ${U(d.total_notional||0)} notional wiped`,
  side:`<div>Last update<br><b>${agoStr(d.last_update||0)}</b> ago</div>`});
 ...
 try{const d=await (await fetch('/api/market-detail?market_id='+encodeURIComponent(mid))).json();
  ...
  h+=`<div class="panel" style="padding:18px 18px 14px">
   <div style="display:flex;align-items:center;gap:14px;margin-bottom:12px;flex-wrap:wrap">
    ...
    <label class="chip" style="cursor:pointer;color:#ffb454;border-color:rgba(255,180,84,.3)" title="Cumulative Volume Delta: running sum of taker-buy − taker-sell notional. Divergence with price hints at hidden pressure."><input id="ch_cvd" type="checkbox" checked style="margin-right:5px;accent-color:#ffb454">CVD</label>
    <label class="chip" style="cursor:pointer" title="Show large trades (>$50k) as dots on the chart, colored by taker direction. Size = log(notional)."><input id="ch_mark" type="checkbox" checked style="margin-right:5px;accent-color:var(--accent2)">Whales</label>
  ...
  const cvdEl=document.getElementById('ch_cvd');if(cvdEl)cvdEl.onchange=()=>{CH_SHOW_CVD=cvdEl.checked;drawCandles();};
  const mkEl=document.getElementById('ch_mark');if(mkEl)mkEl.onchange=()=>{CH_SHOW_MARK=mkEl.checked;drawCandles();};
  loadCandles();
...
let CH_SHOW_CVD=true, CH_SHOW_MARK=true;
async function loadCandles(){
 if(!CH_MID)return;
 try{
  const [dc,dt]=await Promise.all([
   fetch(`/api/candles?market_id=${CH_MID}&interval=${CH_IV}&limit=240`).then(r=>r.json()),
   fetch(`/api/market-ticks?market_id=${CH_MID}&interval=${CH_IV}&limit=240`).then(r=>r.json()).catch(()=>null)
  ]);
  if(!dc.ok||!dc.candles||!dc.candles.length){
   const s=document.getElementById('ch_status');if(s)s.textContent='No data';return;
  }
  CH_DATA=dc.candles;CH_TICKS=dt&&dt.ok?dt:null;
  drawCandles();
  ...
  let extra='';
  if(CH_TICKS){
   const finalCvd=CH_TICKS.cvd_cum[CH_TICKS.cvd_cum.length-1]||0;
   const cvdCls=finalCvd>=0?'pos':'neg';
   extra=` · CVD <span class="${cvdCls}">${finalCvd>=0?'+':''}${U(Math.abs(finalCvd))}</span> · 🐋 ${CH_TICKS.big_trades.length}`;
  }
  document.getElementById('ch_status').innerHTML=
   `<span style="color:var(--accent)">${dc.count} candles</span> · last ${P(last.c)} <span class="${chgCls}">${chg>=0?'+':''}${chg.toFixed(2)}%</span>${extra} · refresh 15s`;
...
function drawCandles(){
 ...
 // ---- CVD overlay (cumulative volume delta) ----
 if(CH_SHOW_CVD&&CH_TICKS&&CH_TICKS.cvd_cum&&CH_TICKS.cvd_cum.length){
  // align: ticks have n_buckets each = bucket_sec; map to candle index by ts
  const cvd=CH_TICKS.cvd_cum;
  // find min/max of CVD to normalize within price area's last ~30% vertical band
  let cmin=Infinity,cmax=-Infinity;
  for(const v of cvd){if(v<cmin)cmin=v;if(v>cmax)cmax=v;}
  if(cmax-cmin>0){
   const band=priceH*0.55;
   const topY=padT+priceH-band;
   const cvd2y=v=>topY+(cmax-v)/(cmax-cmin)*band;
   ctx.strokeStyle='#ffb454';ctx.lineWidth=1.5;ctx.globalAlpha=0.85;
   ctx.beginPath();
   for(let i=0;i<n;i++){
    // alinear el bucket de ticks con el candle por timestamp
    const ct=CH_DATA[i].t;
    const bi=Math.max(0,Math.min(cvd.length-1,Math.floor((ct-CH_TICKS.from_ts)/CH_TICKS.bucket_sec)));
    const y=cvd2y(cvd[bi]);
    const x=padL+i*cw+cw/2;
   ...
   ctx.globalAlpha=1;
   // CVD label
   ctx.fillStyle='#ffb454';ctx.font='10px -apple-system,Inter,sans-serif';ctx.textAlign='left';
   ctx.fillText('CVD',padL+4,topY+10);
  }
 }
 // ---- Trade markers (big trades) ----
 if(CH_SHOW_MARK&&CH_TICKS&&CH_TICKS.big_trades&&CH_TICKS.big_trades.length){
  // map ts to x; price to y
  const firstT=CH_DATA[0].t, intvSec={'1m':60,'5m':300,'15m':900,'1h':3600,'4h':14400,'1d':86400}[CH_IV]||60;
  for(const m of CH_TICKS.big_trades){
   const idx=(m.t-firstT)/intvSec;
   if(idx<0||idx>n-1)continue;
   const x=padL+idx*cw+cw/2;
   const y=px2y(m.price);
   const buy=m.side==='Buy';
   ctx.fillStyle=buy?'rgba(54,211,156,.85)':'rgba(255,84,102,.85)';
   ctx.strokeStyle=buy?'#36d39c':'#ff5466';ctx.lineWidth=1.5;
   // size: log scale
   const r=Math.min(8,3+Math.log10(m.notional/50000)*2);
...
async function testTg(){
 ...
 const ok=await sendTg('✅ <b>RISEx Stats</b>\nTelegram alerts are working!');
