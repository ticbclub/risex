        ...
        try:
            ...
            while b <= latest:
                ...
                for l in logs:
                    ...
                    rec = daily.setdefault(date, {"fees": 0.0, "vol": 0.0})
    ...
    def do_GET(self):
        ...
        try:
            if path.path == "/api/data":
                self._json(_memo("data", 5, get_overview))
            elif path.path == "/api/history":
                self._json(_memo("history", 10, lambda: {"ok": True, "points": load_history()}))
            elif path.path == "/api/cumulative":
                self._json(_memo("cumulative", 60, get_cumulative_growth))
