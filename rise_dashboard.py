...
def db_orders_count(account):
    ...
    try:
        with _DB_LOCK:
            r = _db().execute(
                "SELECT COUNT(DISTINCT order_id) FROM trades WHERE account=? AND order_id IS NOT NULL",
                (account.lower(),)
            ).fetchone()
        return int(r[0]) if r else 0
    except Exception:
...
def _account_metrics(account, now_s, max_pages=None, max_seconds=None):
    ...
    # === INCREMENTAL SCAN ===
    # We persist EVERY fill we fetch into SQLite. On subsequent scans we only fetch
    # trades NEWER than max(ts) we already have. This means a wallet with 100M trades
    # only pays the full scan cost ONCE; future scans are sub-second incremental.
    max_known_ts = db_max_trade_ts(account)
    trade_rows_to_insert = []  # batch insert at end
    ...
    while page <= pages_cap:
        ...
        for t in trades:
            ...
            # Persist this fill to trades table if it's newer than what we have
            if int(ts) > max_known_ts:
            ...
            if ts >= earliest_cutoff:
                ...
                fee = f(t.get("fee"))
                oid = t.get("order_id") or t.get("id")  # fallback to fill id if no order_id
                # ── Sumas de FILLS (correctas a nivel fill: PnL, fees, notional) ──
                if ts >= cutoffs["30d"]:
                    ...
                    # Agrupar por orden para wins/losses + trade count
                    if oid:
        ...
        if not d.get("has_next_page"):
            out["last_refresh"] = now_s  # full scan complete
            break
        # Incremental scan: if the oldest trade in this page is already in our DB,
        # we've reached previously-known data → stop, no need to paginate further
        oldest_in_page = min((int(t.get("time", 0)) / 1e9 for t in trades), default=None)
        if oldest_in_page is not None and max_known_ts > 0 and oldest_in_page <= max_known_ts:
            out["last_refresh"] = now_s  # incremental update complete
            break
        # Also: si la pagina mas reciente ya tiene trades anteriores al cutoff mas antiguo,
        # ya hemos visto todo lo necesario para los stats
        if oldest_in_page is not None and oldest_in_page < earliest_cutoff:
            out["last_refresh"] = now_s  # covered the full 30d window
            break
        page += 1
    else:
        # while-else only fires if VOL_MAX_PAGES exhausted without break — count as full
    ...
    # Convertir order-id groups a contadores finales (1 orden = 1 trade real)
    out["trades"] = len(orders_30d)
    out["trades_7d"] = len(orders_7d)
    out["trades_1d"] = len(orders_1d)
    for oid, rec in orders_30d.items():
        # Win/loss por ORDEN (sumando todos sus fills). Una orden de close con N fills cuenta 1 vez.
        if rec["pnl"] > 0: out["wins_30d"] += 1
        elif rec["pnl"] < 0: out["losses_30d"] += 1
