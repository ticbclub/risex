#!/usr/bin/env python3
"""
RISEx Live Stats Dashboard  ·  v4
=================================
Real-time panel for RISEx (https://www.rise.trade) with:
  - Markets, volume, OI, funding, basis, spread (api.rise.trade)
  - TVL (DefiLlama) and REAL 24h fees by summing onchain events (PerpsManager)
  - Wallet tracker: balance, equity, positions, orders, liq prices, trade history
  - Largest open positions long/short per market (onchain indexer)
  - Account ranking by current total OI (aggregated across all markets)
  - Account ranking by realized VOLUME (1d/7d/14d/30d)
  - Account ranking by average OI (TWAP OI) (1d/7d/14d/30d + custom window)
  - Funding rates comparison against Lighter and Pacifica
  - Users and growth (same microservice as the official explorer)
  - Auto-recorded history, alerts, live activity feed and liquidations
...
VOL_MAX_PAGES = 1000       # max paginas por cuenta (1000 * 1000 trades = hasta 1M trades / 30d)
VOL_REFRESH_S = 600
LARGE_TRADE_USD = 10000    # umbral notional para entrar al feed de "live activity"
FEED_MAX = 1500            # max entradas en el feed global
FEED_WINDOW_S = 86400      # solo guardamos eventos de las ultimas 24h
...
def get_wallet(account):
    ...
    def liq_price(side, entry, leverage, mmf_bps):
        """Liquidation price para posicion ISOLATED. Para cross es aproximada (depende
        del equity total de la cuenta). mmf en basis points (37.5 -> 0.375%)."""
        if not entry or not leverage:
            return None
        mmf = (mmf_bps or 0) / 10000.0
        if side == "Long":
            return entry * (1 - 1 / leverage + mmf)
        else:
            return entry * (1 + 1 / leverage - mmf)
    ...
    try:
        ...
        for p in pd.get("positions", []):
            ...
            side_lbl = "Long" if longp else "Short"
            sign = 1 if longp else -1
            upnl = sign * (mark - entry) * size
            total_upnl += upnl
            lev = f(p.get("leverage")) / WAD
            lp = liq_price(side_lbl, entry, lev, info.get("mmf_bps"))
            dist_to_liq_pct = None
            if lp and mark:
                dist_to_liq_pct = (mark - lp) / mark * 100 if longp else (lp - mark) / mark * 100
            res["positions"].append({
                "market": info.get("name") or mid, "side": side_lbl,
                "size": size, "entry": entry, "mark": mark,
                "notional": size * mark, "upnl": upnl,
    ...
    # Volumen realizado por ventana, del indexer de volumen (que recorre todo el trade-history
    # de cada cuenta y suma price*size). Es mucho mas exacto que sumar solo los 200 trades
    # cargados en esta vista. Si el indexer aun no ha procesado esta cuenta, sera None.
    vol_summary = None
        ...
        with _VOL_LOCK:
            ...
            if v:
                vol_summary = {"1d": v.get("1d", 0), "7d": v.get("7d", 0),
                                "14d": v.get("14d", 0), "30d": v.get("30d", 0),
...
def _account_metrics(account, now_s):
    ...
    events = []
    page = 1
    feed_cutoff = now_s - FEED_WINDOW_S    # solo eventos de las ultimas 24h al feed
    while page <= VOL_MAX_PAGES:
        ...
        for t in trades:
            ...
            if ts >= earliest_cutoff:
                ...
                for k in ("1d", "7d", "14d", "30d", CUSTOM_LABEL):
                    if ts >= cutoffs[k]:
                        out[k] += notional
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=3) as ex:
                for a, v in ex.map(one, accts):
                    all_feed.extend(v.pop("feed_entries", []))
                    with _VOL_LOCK:
                        _VOL["by_account"][a] = v
                        _VOL["scanned"] += 1
            # actualizar feed global: ordenar por ts desc, enriquecer con nombre de mercado
            all_feed.sort(key=lambda e: e["ts"], reverse=True)
            mm = _market_map()
            for e in all_feed:
                e["market"] = (mm.get(e["market_id"]) or {}).get("name") or e["market_id"]
            with _FEED_LOCK:
                _FEED["entries"] = all_feed[:FEED_MAX]
...
def get_oi_ranking(period="1d", limit=200):
    if period not in ("1d", "7d", "14d", "30d", CUSTOM_LABEL):
        period = "1d"
    key = "oi_" + period
    with _VOL_LOCK:
 ...
 <div class="panel">
  ...
  <div class="row">
   <span class="seg" id="vr_seg">
    ...
    <button data-p="14d">14 days</button>
...
<div class="view" id="v_funding">
 <div class="panel">
  ...
  <table id="fc_tbl"><thead><tr>
   <th>Symbol</th><th>RISEx</th><th>Lighter</th><th>Pacifica</th>
...
function renderWalletPage(d){
 ...
 if(s.volume){
  ...
  <div class="cards" style="grid-template-columns:repeat(5,1fr)">
   <div class="card"><div class="lbl">1 day</div><div class="val">${U(v['1d'])}</div></div>
   <div class="card"><div class="lbl">7 days</div><div class="val">${U(v['7d'])}</div></div>
   <div class="card"><div class="lbl">14 days</div><div class="val">${U(v['14d'])}</div></div>
   <div class="card"><div class="lbl">30 days</div><div class="val">${U(v['30d'])}</div><div class="meta">${v.trades_30d} trades · fees ${U(s.fees_30d||0)}</div></div>
   <div class="card"><div class="lbl">Since May 29 07:00 UTC</div><div class="val">${U(v['29may'])}</div><div class="meta">30d realized PnL: <span class="${cf}">${(s.realized_pnl_30d||0)>=0?'+':''}${U(s.realized_pnl_30d||0)}</span>${s.n_liquidations?' · <span class="liq-near">'+s.n_liquidations+' liq.</span>':''}</div></div>
  </div>`;
 } else {
 ...
 if(d.positions.length){
  ...
  h+='</tbody></table></div>';
  h+='<div class="note" style="margin-top:6px">Liq. price is exact for <b>Isolated</b> margin and an approximation for <b>Cross</b> (in cross, actual liquidation depends on total account equity and other positions).</div>';
 } else {
...
function renderFunding(){if(!FC_DATA)return;
 ...
 tb.innerHTML=rows.map(r=>{
  ...
  return `<tr><td class="mkt">${r.symbol}${r.in_risex?'':' <span class="note" style="font-size:10px">(not on RISEx)</span>'}</td>
   <td>${fmtPct(rx)}</td><td>${ltCell}</td><td>${fmtPct(pc)}</td>
   <td>${r.lighter_default?'<span style="color:var(--muted2)">—</span>':fmtPct(dL)}</td><td>${fmtPct(dP)}</td></tr>`;}).join('')||'<tr><td class="empty" colspan=6>No data.</td></tr>';}
