...
VOL_MAX_PAGES = 50         # max paginas por cuenta (50 * 1000 trades = hasta 50k trades en 30d)
VOL_REFRESH_S = 600
...
def fetch_json(url, timeout=15, retries=4):
    """GET con reintentos exponenciales para 429/5xx y errores de red. Critico para no
    perder datos cuando la API rate-limita bajo carga (muchas cuentas en paralelo)."""
    last_err = None
    for i in range(retries):
        try:
            ...
            if e.code in (429, 500, 502, 503, 504) and i < retries - 1:
                time.sleep(0.8 + i * 1.5)   # 0.8, 2.3, 3.8s
                continue
            raise
...
def refresh_ranking():
    ...
    mm = _market_map()
    rows = []
    # 4 workers es buen balance: rapido sin saturar a la API. fetch_json reintenta si la API
    # devuelve 429/5xx, asi que evitamos perder datos silenciosamente como pasaba antes.
...
def volume_indexer_loop():
    ...
    while True:
        try:
            ...
            # 3 workers: balance entre velocidad y no saturar api.rise.trade con miles de cuentas
